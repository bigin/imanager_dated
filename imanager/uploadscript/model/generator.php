...
function prepDataWinDisplay(&$tpl, &$sys)
{
    ...
    /* Check user by IP and generate an user-ident, we'll 
       need it later to create an user specified layout
       to display our thumbnails instead of the full-size images */
    ...
    // Convert $sys['systvs'] to an array and prepare them for further processing
    $arrbuff = explode(',', $sys['systvs']);
    $sys['systvs'] = array();
    foreach($arrbuff as $value)
        $sys['systvs'][] = trim($value);
...
/* This function displays current user images based on 
   UID (userid_date_imagename.jpg [.png] [.gif])
   The Auto-Clean function: images will be deleted 
