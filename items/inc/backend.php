...
function showItemsAdmin($IM, $Reporter)
{
    ...
    if(!isCatValid())
    {
        echo catSelector($Reporter, $Reporter->getClause('items/handles'));
        return false;
    }
    echo catSelector($Reporter, $Reporter->getClause('items/handles'));
...
function showImSettings($Reporter)
{
    ...
    $cf_params_array = im_customfield_def(isset($_POST['cat']) ? $_POST['cat'] : ''); 
...
function returnCustomFields($Reporter)
{
...
function catSelector($Reporter, $noun = 'custom fields')
{   
    global $ISCAT;
...
function isCatValid()
{
    ...
    $category_file = getXML(ITEMDATAFILE);
    foreach($category_file->categories->category as $cat)
        if($cat == htmlspecialchars(stripslashes($_REQUEST['cat']), ENT_QUOTES))
            return true;
    /*if (!in_array(htmlspecialchars(stripslashes($_POST['cat']), ENT_QUOTES), 
        (array)$category_file->categories->category))
        return false;*/
    return false;
}
